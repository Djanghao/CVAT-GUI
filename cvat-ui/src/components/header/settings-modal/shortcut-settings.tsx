...
import { SHORTCUTS_DEFAULT_FEATURE_TOGGLE_STATE } from 'utils/shortcuts-feature-toggles';
...
function ShortcutsSettingsComponent(props: Props): JSX.Element {
    ...
    const items: any = useMemo(() => {
        const scopeItems = Object.values(ShortcutScope)
            .filter((scope: string) => scope !== ShortcutScope.FAST_CVAT)
            .map((scope: string) => {
        ...
        if (filteredScopeItems.length === 0) {
            return null;
        }

        return filteredScopeItems;
    }, [filteredKeyMap, searchValue]);
